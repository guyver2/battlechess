...
class Board(object):
    ...
    def copy(self):
        ...
        res.board = [list(b) for b in self.board]
    ...
    def takeEnPassant(self, i, j, ii, jj):
        if (
            self.board[i][j][0] == "p"
            and jj == self.enpassant
            and (j == self.enpassant - 1 or j == self.enpassant + 1)
        ):
            if (
                self.board[i][j][1] == "w"
                and i == 3
                or self.board[i][j][1] == "b"
                and i == 4
            ):
                ...
                if self.board[i][self.enpassant] == "":
                    print(
                        "Nothing at "
                        + str([i, self.enpassant])
                        + ". Assuming you killed normally"
                    )
                else:
    ...
    def castleInfo(self, piece, i, j, ii, jj):
        if piece in self.castleable:
            if piece[1] == "w":
                if [KWPOS, CASTLEQWPOS] == [
                    [i, j],
                    [ii, jj],
                ] and "rqw" in self.castleable:
                    return "rqw"
                if [KWPOS, CASTLEKWPOS] == [
                    [i, j],
                    [ii, jj],
                ] and "rkw" in self.castleable:
                    return "rkw"
            elif piece[1] == "b":
                if [KBPOS, CASTLEQBPOS] == [
                    [i, j],
                    [ii, jj],
                ] and "rqb" in self.castleable:
                    return "rqb"
                if [KBPOS, CASTLEKBPOS] == [
                    [i, j],
                    [ii, jj],
                ] and "rkb" in self.castleable:
                    return "rkb"
        return ""
    ...
    def getReachablePosition(self, i, j):
        ...
        if c == "pb":
            ...
            if self.isFree(i + 1, j - 1, "b") == 2:
                pos.append([i + 1, j - 1])
            if i == 1 and self.isFree(i + 2, j) == 1 and self.isFree(i + 1, j) == 1:
                pos.append([i + 2, j])
            if (
                self.enpassant != -1
                and i == 4
            ...
            if self.isFree(i - 1, j - 1, "w") == 2:
                pos.append([i - 1, j - 1])
            if i == 6 and self.isFree(i - 2, j) == 1 and self.isFree(i - 1, j) == 1:
    ...
    def move(self, i, j, ii, jj, color=None):
        ...
        reach = self.getReachablePosition(
            i, j
        )  # actually possible destination (obstacles, ennemies)
    ...
    def dump(self, color=None):
        ...
        # hide other player
        if color:
            visibility = [[False for i in range(8)] for j in range(8)]
            for i in range(8):
                for j in range(8):
    ...
    def toString(self, color=None):
        ...
        if color == "b" and visibility[4][self.enpassant] is False:
            enpassantStr = str(-1)
        elif color == "w" and visibility[3][self.enpassant] is False:
            enpassantStr = str(-1)
        else:
            enpassantStr = str(self.enpassant)
        if self.winner:
            winnerStr = self.winner
        else:
            winnerStr = "n"
        res = (
            boardStr
    ...
    def updateFromBoard(self, board):
        ...
        self.board = [list(b) for b in board.board]
    ...
    def updateFromElements(self, board, taken, castleable, enpassant, winner):
        ...
        self.enpassant = ord(enpassant) - ord("a") if enpassant is not None else -1
        self.winner = winner
    ...
    def toElements(self, color=None):
        ...
        elements = {
            "castleable": "".join(
                sorted([self.boardcastle2api()[c] for c in self.castleable])
            ),
