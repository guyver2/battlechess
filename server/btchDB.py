...
class btchDB:
    ...
    def newInvite(self, login, dest, text):
        if self.canInvite(login, dest):
            ...
            if idP1 == None or idP2 == None:
                return False
            self.execute("INSERT INTO invitations (idP1, idP2, message) VALUES ('%s', '%s', '%s')"%(idP1, idP2, text))
            return True
        else: 
            return False
    ...
    def getUserCreatedInvites(self, login, status=None):
        idP1 = self.getUserId(login)
        if status == None:
            self.execute("SELECT id, idP1, idP2, message FROM invitations WHERE idP1=?", (idP1,))
        else:
            self.execute("SELECT id, idP1, idP2, message FROM invitations WHERE idP1=? AND status=?", (idP1,status))
        return self.cursor.fetchall()

    def getUserReceivedInvites(self, login, status=None):
        idP2 = self.getUserId(login)
        if status == None:
            self.execute("SELECT id, idP1, idP2, message FROM invitations WHERE idP2=?", (idP2,))
        else:
            self.execute("SELECT id, idP1, idP2, message FROM invitations WHERE idP2=? AND status=?", (idP2,status))
        return self.cursor.fetchall()
    
    def acceptInvite(self, inviteId):
        self.execute("UPDATE invitations SET status=? where id=?", (btchDB.ACCEPTED, inviteId))
        
    def declineInvite(self, inviteId):
        self.execute("UPDATE invitations SET status=? where id=?", (btchDB.DECLINED, inviteId))
...
class TestDB(unittest.TestCase):
    ...
    def testCreateInvites(self):
        ...
        for i in range(btchDB.MAXINVITE):
            ...
            self.assertEqual(len(self.DB.getUserReceivedInvites("p%d"%(i+1), btchDB.PENDING)), 1)
            self.assertEqual(len(self.DB.getUserReceivedInvites("p%d"%(i+1), btchDB.ACCEPTED)), 0)
            self.assertEqual(len(self.DB.getUserReceivedInvites("p%d"%(i+1), btchDB.DECLINED)), 0)
        self.assertFalse(self.DB.newInvite("p0", "p%d"%(btchDB.MAXINVITE+1), "hey"))
        self.assertEqual(len(self.DB.getUserCreatedInvites("p0", btchDB.PENDING)), btchDB.MAXINVITE)
    ...
    def testUpdateInvites(self):
        ...
        for i in range(1,4):
            ...
            self.assertEqual(len(self.DB.getUserReceivedInvites("p%d"%i, btchDB.PENDING)), 1)
            self.assertEqual(len(self.DB.getUserReceivedInvites("p%d"%i, btchDB.ACCEPTED)), 0)
            self.assertEqual(len(self.DB.getUserReceivedInvites("p%d"%i, btchDB.DECLINED)), 0)
