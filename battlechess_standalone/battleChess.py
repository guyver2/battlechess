...
class BoardPlayer(Board):
    ...
    def draw(self, screen, selected=None, turn=None):
        ...
        for i in range(8):
            for j in range(8):
                ...
                if c == "":
                    ...
                    screen.blit(
                        self.sprite_pieces, (104 + j * 40, 30 + i * 40), patch_rect
                    )
        if turn is not None:
        ...
        for i, p in enumerate([p for p in self.taken if p[1] == "w"]):
            ...
            patch_rect = (dx, dy, 36, 36)
            screen.blit(
                self.sprite_pieces, (6 + (i % 2) * 40, 20 + (i / 2) * 40), patch_rect
            )
        for i, p in enumerate([p for p in self.taken if p[1] == "b"]):
            ...
            patch_rect = (dx, dy, 36, 36)
            screen.blit(
                self.sprite_pieces, (426 + (i % 2) * 40, 338 - (i / 2) * 40), patch_rect
            )
        ...
        if turn != localPlayer:
            ...
            if mpos:
                ...
                if clickCell:
                    ...
                    if cell:
                        sendData(
                            sock, "MOVE", [clickCell[0], clickCell[1], cell[0], cell[1]]
                        )
...
def endGameState(screen, winner, localPlayer, board):
    ...
    while loop:
        ...
        screen.blit(
            text,
            (
                int(W / 2.0 - text.get_width() / 2.0),
                int(H / 2.0 - text.get_height() / 2.0),
            ),
        )
